...
CREATE INDEX idx_kb_articles_embedding ON knowledge_base.articles USING ivfflat(embedding_vector vector_cosine_ops) WITH (lists = 100);
...
COMMENT ON SCHEMA knowledge_base IS 'Knowledge base for support articles and documentation';
COMMENT ON TABLE knowledge_base.articles IS 'Core KB articles with full-text and semantic search';
...
COMMENT ON COLUMN knowledge_base.articles.embedding_vector IS 'OpenAI embedding (1536 dimensions) for semantic search';
