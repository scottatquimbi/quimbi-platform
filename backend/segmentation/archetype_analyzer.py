...
This module provides tools to analyze customer archetypes using both
...
# Import CustomerMultiAxisProfile from clustering engine
import sys
import os
...
class Archetype:
    """
    Customer archetype with count and metadata.
...
class ArchetypeAnalyzer:
    """
    Analyzes customer archetypes with fuzzy membership support.
    ...
    def create_strength_signature(profile) -> ArchetypeSignature:
        ...
        This groups customers by dominant segment + strength (strong/balanced/weak),
        creating practical cohort sizes without losing behavioral nuance.

        Args:
            profile: CustomerMultiAxisProfile with membership_strength
    ...
    def create_fuzzy_signature(profile, rounding: int = 1) -> ArchetypeSignature:
        """
        Create fuzzy archetype signature from customer profile.

        Args:
            profile: CustomerMultiAxisProfile with fuzzy_memberships and top2_segments
    ...
    def count_archetypes(
        ...
        """
        Count unique archetypes across customers at specified granularity level.

        Args:
            profiles: List of CustomerMultiAxisProfile objects
        ...
        """
        archetype_counter = defaultdict(lambda: {"count": 0, "customer_ids": []})
        ...
        for profile in profiles:
            ...
            archetype_counter[sig]["customer_ids"].append(profile.customer_id)
        ...
        for sig, data in archetype_counter.items():
            archetypes[sig] = Archetype(
                ...
                population_percentage=data["count"] / total_customers if total_customers > 0 else 0,
    ...
    def get_archetype_statistics(archetypes: Dict[ArchetypeSignature, Archetype]) -> Dict:
        ...
        """
        counts = [arch.customer_count for arch in archetypes.values()]
        ...
        return {
            ...
            "singleton_archetypes": sum(1 for c in counts if c == 1),  # 1 customer
            "small_archetypes": sum(1 for c in counts if 2 <= c <= 5),  # 2-5 customers
            "medium_archetypes": sum(1 for c in counts if 6 <= c <= 10),  # 6-10 customers
            "large_archetypes": sum(1 for c in counts if c > 10),  # >10 customers
    ...
    def get_top_archetypes(
        ...
        Returns:
            List of Archetype objects sorted by customer_count (descending)
        """
        sorted_archetypes = sorted(
            archetypes.values(),
            key=lambda a: a.customer_count,
            reverse=True
        )
        return sorted_archetypes[:top_n]

    @staticmethod
    def find_customer_archetype(
        customer_id: str,
        archetypes: Dict[ArchetypeSignature, Archetype]
    ) -> Optional[Archetype]:
        """
        Find the archetype a customer belongs to.
        ...
        for archetype in archetypes.values():
            if customer_id in archetype.customer_ids:
                return archetype
        return None
    ...
    def export_archetypes_to_dict(
        archetypes: Dict[ArchetypeSignature, Archetype],
        include_customer_ids: bool = False
    ) -> List[Dict]:
        """
        ...
        Args:
            archetypes: Dictionary of archetypes
            include_customer_ids: Whether to include full list of customer IDs

        Returns:
            List of archetype dictionaries
        """
        result = []
        for archetype in sorted(archetypes.values(), key=lambda a: a.customer_count, reverse=True):
            arch_dict = archetype.to_dict()
            if not include_customer_ids:
                arch_dict.pop("customer_ids", None)
            else:
                arch_dict["customer_ids"] = archetype.customer_ids
